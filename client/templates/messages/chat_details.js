...
function MsgInfoCtrl (
  ...
  this.helpers({
    ...
    uploadImg: () => Uploads.findOne({ listID: self.getReactively('chat.listingID') }),
    ...
    userImg: () => ProfileImg.findOne({
      $and: [{
        ...
        $or:[{
          userId: self.getReactively('chat.buyer')
        },{
          userId: self.getReactively('chat.seller')
        }]
      }]
    })
  });
